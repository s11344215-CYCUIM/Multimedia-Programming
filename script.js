...
function openOrderModal(drinkId, drinkName, drinkPrice) {
    ...
    if (modalDrinkPriceEl) modalDrinkPriceEl.textContent = `價格：$${drinkPrice || 60}`;
...
if (addToCartButtons.length) {
    addToCartButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // 大杯 +10 元
            if (size === '大杯（+$10）' || size === '大杯(+10)' || size === '大杯（+$10）') {
                price += 10;
            }
...
function renderCheckoutPage() {
    if (!checkoutContentEl || !checkoutEmptyEl2 || !checkoutSummaryEl || !checkoutTotalEl) return;
    ...
    cart.forEach(item => {
        ...
        <div class="checkout-summary-item">
            <div class="checkout-summary-item-main">
                <div class="checkout-summary-item-name">${item.name} x ${item.quantity}</div>
                <div class="checkout-summary-item-options">${optionsText || ''}</div>
            </div>
...
function getRatingInfo(drinkId, userId) {
    ...
    const userRating = userId && entry.userRatings ? entry.userRatings[userId] || null : null;
    const comment = userId && entry.userComments ? entry.userComments[userId] || '' : '';
...
function openRatingModalForBlock(block, drinkId) {
    ...
    if (ratingModalStarsEl) {
        ratingModalStarsEl.querySelectorAll('.rating-star').forEach(star => {
            const val = Number(star.dataset.star) || 0;
            star.classList.toggle('active', val <= currentRatingValue && currentRatingValue > 0);
        });
    }
